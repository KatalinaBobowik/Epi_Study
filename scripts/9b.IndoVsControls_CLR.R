...
# Indonesian RPM
load(paste0(inputdir,"AllREadsSE_Indo_RPM_physeq.Rda"))
# control counts
...
merged_phylo_RPM=merge_phyloseq(AllREadsSE_Indo_RPM_physeq, control_100K_RPM_physeq)
...
otu_table(merged_phylo_RPM)[otu_table(merged_phylo_counts)<=1]<-0
...
merged_phylo_RPM <- prune_taxa(taxa_sums(merged_phylo_RPM) > 0, merged_phylo_RPM)

# offset
otu_table(merged_phylo_counts)=otu_table(merged_phylo_counts)+1
result.filter=t(otu_table(merged_phylo_counts))
...
# result.filter=low.count.removal(result.filter, percent=0.01)
data.clr <- logratio.transfo(as.matrix(result.filter), logratio = 'CLR', offset = 0) 
pca.clr <- pca(data.clr)
plotIndiv(pca.clr, title = 'My second PCA plot')
...
mergedlog <- transform_sample_counts(merged_phylo_counts_rarefied, function(x) log(1 + x))
...
taxa = otu_table(t(data.clr), taxa_are_rows = TRUE)
otu_table(merged_phylo_counts)=taxa

pAlpha = plot_richness(merged_phylo_counts,
                       color = "SamplePop",
...
ps4 <- core(merged_phylo_counts, detection = 1, prevalence = 2/nsamples(merged_phylo_counts))

ps4.rel <- microbiome::transform(ps4, "compositional")
bx.ord_pcoa_bray <- ordinate(merged_phylo_counts, "PCoA", "bray")

# Make an ordination plot using bray's dissimilarity
beta.ps1 <- plot_ordination(data.clr, 
                            bx.ord_pcoa_bray, 
                            color="SamplePop", 
                            label = "SampleName") + 
...
---
# following this: https://joey711.github.io/phyloseq-extensions/DESeq2.html
otu_table(merged_phylo_counts)=otu_table(merged_phylo_counts)+1
taxa_names(merged_phylo_counts)=make.unique(tax_table(merged_phylo_counts)[,"Phylum"])
diagdds = phyloseq_to_deseq2(merged_phylo_counts, ~ SamplePop)
...
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(merged_phylo_counts)[rownames(sigtab), ], "matrix"))
head(sigtab)
