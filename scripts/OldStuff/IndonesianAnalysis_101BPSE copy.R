...
# try plotting with microbiome clr

ps_clr <- microbiome::transform(AllREadsSE_Indo_Counts_physeq, "clr")
ord_clr <- phyloseq::ordinate(AllREadsSE_Indo_Counts_physeq, method = "PCoA", distance = "euclidean")
phyloseq::plot_ordination(ps_clr, ord_clr, type="samples", color="SamplePop", label="SampleName", axes = 1:2) + 
  geom_point(size = 2) + scale_colour_manual(values=c(KorowaiCol,MentawaiCol,SumbaCol))
...
save(AllREadsSE_Indo_Counts_physeq_clr, file=paste0(outputdir,"AllREadsSE_Indo_Counts_physeq_clr.Rda"))

out.wuf.log <- ordinate(AllREadsSE_Indo_Counts_physeq_clr, method = "PCoA", distance = "euclidean")
pdf(paste0(outputdir,"PCA_FirstDimension_CLRTransform.pdf"))
...
# alternatively, you can plot with ellipses
# Make an ordination plot using euclidean distances
beta.ps1 <- plot_ordination(AllREadsSE_Indo_Counts_physeq_clr, 
                            out.wuf.log, 
                            color="SamplePop", 
                            label = "SampleName", axes = 1:2) + 
  geom_point(aes(), size= 4) + 
  theme(plot.title = element_text(hjust = 0, size = 12))

# add in an ellipse
pdf(paste0(outputdir,"PCA_FirstDimension_CLRTransform_WithEllipse_ByIsland.pdf"))
beta.ps1 + stat_ellipse() + theme_bw(base_size = 14) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  scale_colour_manual(values=c(KorowaiCol,MentawaiCol,SumbaCol))
dev.off()

# plot without ellipse
pdf(paste0(outputdir,"PCA_FirstDimension_CLRTransform_noEllipse_ByIsland.pdf"))
beta.ps1 + theme_bw(base_size = 14) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  scale_colour_manual(values=c(KorowaiCol,MentawaiCol,SumbaCol))
dev.off()

# we can also plot this by batch
beta.ps1 <- plot_ordination(AllREadsSE_Indo_Counts_physeq_clr, 
                            out.wuf.log, 
                            color="batch", 
                            label = "SampleName") + 
...
# add in an ellipse
pdf(paste0(outputdir,"PCA_FirstDimension_CLRTransform_WithEllipse_ByBatch.pdf"))
beta.ps1 + stat_ellipse() + theme_bw(base_size = 14) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  scale_colour_manual(values=c("#332288","#AA4499","black"))
dev.off()

# In the first principal component, there isn't any clear grouping by island, but you can see grouping by batch in the second dimension.
# In PC1, some samples can be seen clustering away from the majority of the samples. These are samples with a high Plasmodium load. 
# Let's highlight each sample by Plasmodium load (on the log scale for easier visualisation).
...
out.wuf.log <- ordinate(AllREadsSE_Indo_Counts_physeq_clr, method = "PCoA", distance = "euclidean")
pdf(paste0(outputdir,"PCA_FirstDimension_Apicomplexa_CLRTransform.pdf"))
...
# add in an ellipse
pdf(paste0(outputdir,"PCA_FirstDimension_CLRTransform_WithEllipse_ByApicomplexa.pdf"))
beta.ps1 + stat_ellipse() + theme_bw(base_size = 14) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
dev.off()

# plot without an ellipse
pdf(paste0(outputdir,"PCA_FirstDimension_CLRTransform_WithEllipse_ByApicomplexa.pdf"))
beta.ps1 + theme_bw(base_size = 14) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
dev.off()
...
out.wuf.log <- ordinate(AllREadsSE_Indo_Counts_physeq_clr, method = "PCoA", distance = "euclidean")
pdf(paste0(outputdir,"PCA_Dimensions2to5_CLRTransform.pdf"))
...
#Plot
pdf(paste0(outputdir,"hierarchicalSlutering_IslandLabels.pdf"))
plot(ward)
dev.off()
...
pdf(paste0(outputdir,"hierarchicalSlutering_ApicomplexaLabels.pdf"))
plot(ward)
dev.off()
