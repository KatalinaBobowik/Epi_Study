...
rule all:
        input:
                ...
                expand("star/firstpass/{sample}_SJ.out.tab", sample=config["samples"]),
...
rule download_files:
        ...
        log:
                "logs/fasterqDump/{sample}.log"
        shell:
                "module load web_proxy; module load SRA-Toolkit; fasterq-dump {wildcards.sample} -O /scratch/punim0586/kat/Bipolar_controlSamples_EpiStudy/{wildcards.sample}/ &> {log}"
...
rule star_pe_firstmap:
        ...
        output:
                ...
                "star/firstpass/{sample}_SJ.out.tab"
        log:
                "logs/star/{sample}.log"
        shell:
                # unfortunately I have no better way of doing it- define sample name then use that as dirname
                "module load web_proxy; module load STAR; x=`basename {input.f1} _1.fastq`; STAR --genomeDir {input.dir} "
...
rule star_index_second:
        input:
                fasta="/data/cephfs/punim0586/shared/genomes/hg38/Homo_sapiens.GRCh38.p10.ensemblv90.dna.primary_assembly.fa",
                gtf="/data/cephfs/punim0586/shared/genomes/hg38/GTF_annotation/Homo_sapiens.GRCh38.90.gtf",
                sjdb="star/STAR_Hg38_allFilesSJ.out.tab"
        output:
                directory("genome_secondPass")
        shell:
                "module load web_proxy; module load STAR; mkdir genome_secondPass; STAR --runMode genomeGenerate "
                ...
                "--sjdbFileChrStartEnd {input.sjdb} " #this is the file we used to get all of the combined splice junctions
                "--runThreadN 12"
...
rule star_pe_secondmap:
        input:
                f1="/scratch/punim0586/kat/Bipolar_controlSamples_EpiStudy_Trimmed/{sample}_1.fastq",
                f2="/scratch/punim0586/kat/Bipolar_controlSamples_EpiStudy_Trimmed/{sample}_2.fastq",
                dir=directory("genome_secondPass")
        output:
