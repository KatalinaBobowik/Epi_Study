...
downsampled_diversity_test <- function(subsample, iteration) {
  ...
  # Run divnet
  dv_pop_comparison <- estimate_richness(pop_comparison)
...
getEstimate_Shannon = function(population){
  sapply(1:1000, function(x) mean(as.matrix(filter(as.data.frame(downsampledSummary[[x]]$Shannon) %>% add_column("SamplePop" = c(rep("Indonesia",12),rep("Mali",12),rep("UK",12))), SamplePop == population)[1])))
}
