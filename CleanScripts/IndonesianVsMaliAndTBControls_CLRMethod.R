...
# We can see that the first principal component is separating the three studies apart, however there is a clear outlier in the UK population - SRR6369904. We can see this even more clearly in the third PC. 
...
# After removing this sample, let's check the library sizes for each of our populations.

table(sample_data(merged_phylo_counts)[,"SamplePop"])

# We now have 117 Indonesian samples, 54 Malian samples, and 9 UK samples. 
# Now we can replot the data. Let's have a look at the first five PCs to explore the data.
...
# As seen in the first figure, PC1 separates groups by study. However in PC4, we can see some of the Indonesian samples with a high Plasmodium load separating away from the rest of the Indonesian samples. To better visualise this, let's highlight each sample by Plasmodium load (on the log scale for easier visualisation).
...
# You can now see lighter blue samples (high Plasmodium load) separating away from darker blue samples (no Plasmodium load).
