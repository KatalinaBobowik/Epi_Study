...
```{r, message = FALSE}
merged_phylo_counts=merge_phyloseq(AllREadsSE_Indo_Counts_physeq, control_100K_Counts_physeq, Mali_Counts_physeq, Americans_Counts_physeq)
merged_phylo_counts
...
scale_fill_manual(values = c(IndonesiaCol,NetherlandsCol,MaliCol,USACol)) + rotate_x_text()
```
...
plot_ordination(merged_phylo_counts_clr, out.wuf.log, color="SamplePop", axes = 1:2, label="SampleName") + scale_colour_manual(values=c(IndonesiaCol,NetherlandsCol,MaliCol,USACol))
```
...
```{r, message = FALSE}
plot_ordination(merged_phylo_counts_clr, out.wuf.log, color="SamplePop", axes = 3:4, label="SampleName") + scale_colour_manual(values=c(IndonesiaCol,NetherlandsCol,MaliCol,USACol))
```
...
p1=plot_ordination(mergedlog, out.wuf.log, type="taxa", color="Phylum", title="taxa", axes = 1:2)
...
colorCode <- c(Indonesia = IndonesiaCol, `Netherlands` = NetherlandsCol, `Mali` = MaliCol, `America` = USACol)
...
```{r, message = FALSE}
differential_abund=merged_phylo_counts
...
EnhancedVolcano(sigtab, x = 'log2FoldChange', y = 'padj', lab=rownames(sigtab), pCutoff = 0.05, FCcutoff = 1)

ggplot(sigtab) +
  geom_point(aes(x = log2FoldChange, y = -log10(padj))) +
  # geom_text_repel(aes(x = log2FoldChange, y = -log10(padj), label = rownames(sigtab),""))) +
...
adiv %>%
  ...
  theme(legend.position="none") + scale_colour_manual(values=c(IndonesiaCol,NetherlandsCol,MaliCol,USACol))
...
phyloseq::plot_ordination(pruned_data, ord_clr, type="samples", color="SamplePop",label = "SampleName") + 
  ...
  stat_ellipse(aes(group = SamplePop), linetype = 2) + scale_colour_manual(values=c(IndonesiaCol,NetherlandsCol,MaliCol,USACol))
...
beta.ps1 + stat_ellipse() + theme_bw(base_size = 14) + 
  ...
  scale_colour_manual(values=c(IndonesiaCol,NetherlandsCol,MaliCol,USACol))
```
