...
control_100K_Counts <- read.csv(paste0(refdir,"test.csv"),check.names=FALSE)
...
# add sequencing depth information before filtering
SeqDepth_Prefilter = colSums(otu_table(control_100K_Counts_physeq))
sample_data(control_100K_Counts_physeq)$SeqDepth_Prefilter = SeqDepth_Prefilter
...
We'll also be documenting how many reads we have at each step of the pipeline, so let's add that to a temporary dataframe.
...
temp_merged_phylo_counts=merge_phyloseq(AllREadsSE_Indo_Counts_physeq, Mali_Counts_physeq, control_100K_Counts_physeq, Americans_Counts_physeq)

# histogram of data
ggplot(meta(temp_merged_phylo_counts)) + geom_histogram(aes(x = SeqDepth_Prefilter), alpha= 0.6, bins=100) + facet_wrap(~SamplePop)
...
# Separate species' abundances and taxonomy columns
rarecurve_counts <- otu_table(temp_merged_phylo_counts)
col <- c(rep(IndonesiaCol,sum(sample_data(temp_merged_phylo_counts)[,"SamplePop"]=="Indonesia")),rep(MaliCol,sum(sample_data(temp_merged_phylo_counts)[,"SamplePop"]=="Mali")),rep(NetherlandsCol,sum(sample_data(temp_merged_phylo_counts)[,"SamplePop"]=="Netherlands")), rep(USACol,sum(sample_data(temp_merged_phylo_counts)[,"SamplePop"]=="USA")))
...
# add sequencing depth information before filtering
SeqDepth_noSingletons = colSums(otu_table(Mali_Counts_physeq))
sample_data(Mali_Counts_physeq)$SeqDepth_noSingletons = SeqDepth_noSingletons

otu_table(control_100K_Counts_physeq)[otu_table(control_100K_Counts_physeq)<=1]<-0
control_100K_Counts_physeq <- prune_taxa(taxa_sums(control_100K_Counts_physeq) > 0, control_100K_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noSingletons = colSums(otu_table(control_100K_Counts_physeq))
sample_data(control_100K_Counts_physeq)$SeqDepth_noSingletons = SeqDepth_noSingletons

otu_table(Americans_Counts_physeq)[otu_table(Americans_Counts_physeq)<=1]<-0
Americans_Counts_physeq <- prune_taxa(taxa_sums(Americans_Counts_physeq) > 0, Americans_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noSingletons = colSums(otu_table(Americans_Counts_physeq))
sample_data(Americans_Counts_physeq)$SeqDepth_noSingletons = SeqDepth_noSingletons
```
...
Mali_Counts_physeq <- prune_taxa(taxa_sums(Mali_Counts_physeq) > 0, Mali_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noViridiplantae = colSums(otu_table(Mali_Counts_physeq))
sample_data(Mali_Counts_physeq)$SeqDepth_noViridiplantae = SeqDepth_noViridiplantae

control_100K_Counts_physeq <- subset_taxa(control_100K_Counts_physeq, (Kingdom!="Viridiplantae"))
control_100K_Counts_physeq <- prune_taxa(taxa_sums(control_100K_Counts_physeq) > 0, control_100K_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noViridiplantae = colSums(otu_table(control_100K_Counts_physeq))
sample_data(control_100K_Counts_physeq)$SeqDepth_noViridiplantae = SeqDepth_noViridiplantae

Americans_Counts_physeq <- subset_taxa(Americans_Counts_physeq, (Kingdom!="Viridiplantae"))
Americans_Counts_physeq <- prune_taxa(taxa_sums(Americans_Counts_physeq) > 0, Americans_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noViridiplantae = colSums(otu_table(Americans_Counts_physeq))
sample_data(Americans_Counts_physeq)$SeqDepth_noViridiplantae = SeqDepth_noViridiplantae
...
Mali_Counts_physeq <- prune_taxa(taxa_sums(Mali_Counts_physeq) > 0, Mali_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noMetazoa = colSums(otu_table(Mali_Counts_physeq))
sample_data(Mali_Counts_physeq)$SeqDepth_noMetazoa = SeqDepth_noMetazoa

control_100K_Counts_physeq <- subset_taxa(control_100K_Counts_physeq, (Kingdom!="Metazoa"))
control_100K_Counts_physeq <- prune_taxa(taxa_sums(control_100K_Counts_physeq) > 0, control_100K_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noMetazoa = colSums(otu_table(control_100K_Counts_physeq))
sample_data(control_100K_Counts_physeq)$SeqDepth_noMetazoa = SeqDepth_noMetazoa

Americans_Counts_physeq <- subset_taxa(Americans_Counts_physeq, (Kingdom!="Metazoa"))
Americans_Counts_physeq <- prune_taxa(taxa_sums(Americans_Counts_physeq) > 0, Americans_Counts_physeq)
# add sequencing depth information before filtering
SeqDepth_noMetazoa = colSums(otu_table(Americans_Counts_physeq))
sample_data(Americans_Counts_physeq)$SeqDepth_noMetazoa = SeqDepth_noMetazoa
...
```{r, message = FALSE}
temp_merged_phylo_counts=merge_phyloseq(AllREadsSE_Indo_Counts_physeq, Mali_Counts_physeq, control_100K_Counts_physeq, Americans_Counts_physeq)
FilteringSummary = sample_data(temp_merged_phylo_counts)[,c("SamplePop","SeqDepth_Prefilter","SeqDepth_noSingletons","SeqDepth_noViridiplantae","SeqDepth_noMetazoa")]
...
# AllREadsSE_Indo_Counts_physeq <- tax_glom(AllREadsSE_Indo_Counts_physeq, "Family")
# any(taxa_sums(AllREadsSE_Indo_Counts_physeq) == 0)
...
# Mali_Counts_physeq <- tax_glom(Mali_Counts_physeq, "Family")
...
# control_100K_Counts_physeq <- tax_glom(control_100K_Counts_physeq, "Family")
# any(taxa_sums(control_100K_Counts_physeq) == 0)
...
# Americans_Counts_physeq <- tax_glom(Americans_Counts_physeq, "Family")
...
taxa_names(control_100K_Counts_physeq) <- make.unique(paste(tax_table(control_100K_Counts_physeq)[,"Superkingdom"], tax_table(control_100K_Counts_physeq)[,"Kingdom"], tax_table(control_100K_Counts_physeq)[,"Phylum"], tax_table(control_100K_Counts_physeq)[,"Class"], tax_table(control_100K_Counts_physeq)[,"Order"], tax_table(control_100K_Counts_physeq)[,"Family"], tax_table(control_100K_Counts_physeq)[,"Genus"], tax_table(control_100K_Counts_physeq)[,"Species"], sep="_"))
taxa_names(Americans_Counts_physeq) <- paste(tax_table(Americans_Counts_physeq)[,"Superkingdom"], tax_table(Americans_Counts_physeq)[,"Kingdom"], tax_table(Americans_Counts_physeq)[,"Phylum"], tax_table(Americans_Counts_physeq)[,"Class"], tax_table(Americans_Counts_physeq)[,"Order"], tax_table(Americans_Counts_physeq)[,"Family"], tax_table(Americans_Counts_physeq)[,"Genus"], tax_table(Americans_Counts_physeq)[,"Species"], sep="_")
...
# add sequencing depth information after agglomeration
SeqDepth_Agglomerate = colSums(otu_table(merged_phylo_counts))
sample_data(merged_phylo_counts)$SeqDepth_Agglomerate = SeqDepth_Agglomerate

# make a barplot of library sizes
ggplot(meta(merged_phylo_counts), aes(SampleName, SeqDepth_Agglomerate)) + geom_bar(stat = "identity", aes(fill = SamplePop)) + scale_fill_manual(values = c(IndonesiaCol,MaliCol,NetherlandsCol,USACol)) + rotate_x_text()
```
...
# For some reason, top is actually top + 1, so here it would be 20
aggregated_phyloCounts <- aggregate_top_taxa(pruned_data, "Phylum", top = 20)
# transform to relative counts
relative_phyloCounts <- microbiome::transform(aggregated_phyloCounts, "compositional")
